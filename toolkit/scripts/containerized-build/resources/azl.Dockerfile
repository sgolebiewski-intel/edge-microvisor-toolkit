...
RUN echo "echo installing packages edge-release vim git ${extra_packages}" >> /root/.bashrc && \
    echo "tdnf install --releasever=${version} -qy edge-release vim git ${extra_packages}" >> /root/.bashrc
