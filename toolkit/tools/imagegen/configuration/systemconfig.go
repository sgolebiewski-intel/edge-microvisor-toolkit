...
type SystemConfig struct {
	...
	EfiProvider            string                    `json:"EfiProvider"`
	ImmutabilityEnabled    bool                      `json:"ImmutabilityEnabled"`
	EMTEncryptionEnabled   bool                      `json:"EMTEncryptionEnabled"`
}
...
func (s *SystemConfig) IsValid() (err error) {
	...
	// Validate EfiProvider
	if s.EfiProvider != "grub-efi" && s.EfiProvider != "systemd-boot" && s.EfiProvider != "none" && s.EfiProvider != "" {
		return fmt.Errorf("invalid [EfiProvider]: %s. Expecting values of either 'grub-efi', or 'systemd-boot'", s.EfiProvider)
	}

	// Validate ImmutabilityEnabled
	if s.ImmutabilityEnabled {

		if s.EfiProvider != "systemd-boot" {
			return fmt.Errorf("invalid [EfiProvider]: %s. Expecting value is 'systemd-boot' when ImmutabilityEnabled is true", s.EfiProvider)
		}

		// Check if layout.env is being installed, when Immutability is enabled.
		var isLayoutCfgPresent bool
		for srcFile, fileConfigList := range s.AdditionalFiles {
			err = fileConfigList.IsValid()
			if err != nil {
				return fmt.Errorf("invalid [AdditionalFiles]: (%s): %w", srcFile, err)
			...
			// Check if layout.env is being installed, when Immutability is enabled.
			if strings.Contains(srcFile, "layout.env") {
				isLayoutCfgPresent = true
				break
			}
		}
		if !isLayoutCfgPresent {
			return fmt.Errorf("Expecting layout.env to be present in [AdditionalFiles], when ImmutabilityEnabled is true")
		}

		// Check if persistent-mount-package.json is being installed, when Immutability is enabled.
		var isPkgPresent bool
		for _, file := range s.PackageLists {
			if strings.Contains(file, "persistent-mount-package") {
				isPkgPresent = true
				break
			}
		}
		if !isPkgPresent {
			return fmt.Errorf("Expecting persistent-mount-package to be installed in [PackageLists], when ImmutabilityEnabled is true")
		}

	}
	// Validate EMTEncryptionEnabled
	if s.EMTEncryptionEnabled {
		if s.EfiProvider != "systemd-boot" {
			return fmt.Errorf("invalid [EfiProvider]: %s. Expecting value is 'systemd-boot' when EMTEncryptionEnabled is true", s.EfiProvider)
		}
		if !s.ImmutabilityEnabled {
			return fmt.Errorf("invalid [ImmutabilityEnabled]: %v. Expecting value is 'true' when EMTEncryptionEnabled is true", s.ImmutabilityEnabled)
		}
		// Check if fde-verity-package.json is being installed, when fde and verity is enabled.
		var isPkgPresent bool
		for _, file := range s.PackageLists {
			if strings.Contains(file, "fde-verity-package") {
				isPkgPresent = true
				break
			}
		}
		if !isPkgPresent {
			return fmt.Errorf("Expecting fde-verity-package to be installed in [PackageLists], when EMTEncryptionEnabled is true")
		}
	}
