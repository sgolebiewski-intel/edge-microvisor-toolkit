...
%if 0%{?enable_gstreamer}
BuildRequires:  orc-devel
%endif
%if 0%{?database_stdb}
BuildRequires:  libtdb-devel
%endif
%if 0%{?enable_speexdsp}
...
%meson \
  ...
  -D lirc=disabled \
  -D tcpwrap=disabled \
  -D bluez5=%{?enable_bluetooth:enabled}%{!?enable_bluetooth:disabled} \
  ...
  -D gtk=disabled \
  -D soxr=disabled \
  -D fftw=%{?enable_fftw:enabled}%{!?enable_fftw:disabled} \
  -D asyncns=%{?enable_asyncns:enabled}%{!?enable_asyncns:disabled} \
  -D speex=%{?enable_speexdsp:enabled}%{!?enable_speexdsp:disabled} \
